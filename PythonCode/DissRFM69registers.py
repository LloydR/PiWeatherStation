...
RF_BITRATEMSB_19200 = 0x06 #Davis
...
#From https://github.com/dekay/DavisRFM69/wiki/RF-Protocol
#Packets are sent from the ISS every 2.5 seconds for an ISS set to a transmit ID of zero.
#The rate gets slower as the transmit ID increases by 1/16 of a second for every station ID number
#e.g. ID 1 transmits at an interval of 2.5625 seconds (ref: Davis Serial Protocol document). 
#The data rate is 19.2 kbps and is transmitted from the ISS with the least significant bit first.

# RegFdev - frequency deviation (Hz)
#define RF_FDEVMSB_4800     0x00 // Used for Davis console reception  Need to add these
#define RF_FDEVLSB_4800     0x4e

RF_FDEVMSB_2000 = 0x00
RF_FDEVLSB_2000 = 0x21
RF_FDEVMSB_4800 = 0x00  #Davis
...
RF_PREAMBLESIZE_LSB_DAVIS = 0x04
...
#ISS has fixed packet length of 10 bytes, including CRC and retransmit CRC
DAVIS_PACKET_LEN = 10
